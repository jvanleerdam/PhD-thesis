...
\end{equation}
where $\eta_i$\texteq\tpm1 is the CP eigenvalue of the state, $\lamfAbs[i]$\texteq$\qpAbsAlt\big|\frac{\Abari}{\Ai}\big|$, and the
CP-violating phase $\phisi$ is given by $\phisi\text{\textequiv\tm}\arg\big(\frac{1}{\eta_i}\lamf[i]\big)$. If CP symmetry is violated by
the same amount for all intermediate states, the combination $\frac{1}{\eta_i}\lamf[i]$ does not depend on the state and $\phisi\to\phis$.
