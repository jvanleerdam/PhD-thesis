...
either $\Bsst$ or $\Bsbarst$, the required time-dependent states are given by Equation~\ref{eq:timeEvolBBbarStateInit}. Using the
orthogonality of $\Bsst$ and $\Bsbarst$ and labelling the amplitude of a decay of $\Bsst$ into a final state $\fst$ by $\Af$, the amplitude
for a system produced as a $\Bs$, being in a $\Bs$ state at the time of decay and decaying into $\fst$ is given by
$\langle\Bs|\Psi_{\Bs}(t)\rangle\,\Af \propto g_+(t)\,\Af$.
...
and multiplying with a factor $\pq$ $\big(\qp\big)$.
...
$\cDms$ and $\sDms$ terms. For the $\fbarst$ final state, the parameter $\lamf$ goes to $\lamfbar$, which is defined as
\begin{equation}
  \lamfbar\equiv\pq\frac{\Afbar}{\Abarfbar}
  \ .
\end{equation}
With $\lamfbar$ parameters $\Cfbar$, $\Dfbar$ and $\Sfbar$ are defined similar to $\Cf$, $\Df$ and $\Sf$ in Equation~\ref{eq:mixDecayCDS}
with $\lamf$.
