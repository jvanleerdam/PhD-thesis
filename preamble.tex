...
\cfoot{}
\makeatletter
\newcommand{\defaultchaptermark}[1]{\markboth{\@chapapp\ \thechapter. \ #1}{}}
\makeatother
\newcommand{\defaultsectionmark}[1]{\markright{\thesection. \ #1}}
...
\hyphenation{ver-val-len}
\hyphenation{nauw-keu-rige}
\hyphenation{ele-men-taire}
\hyphenation{deel-tjes-fy-si-ca}
\hyphenation{voor-spel-ling-en}
\hyphenation{be-schrij-ving}
\hyphenation{ge-bon-den}
\hyphenation{schoon-heids-me-so-nen}
\hyphenation{vor-men}
\hyphenation{aan-van-ke-lijke}
\hyphenation{bij-na}
\hyphenation{waar-schijn-lijk-heids-am-pli-tu-de}
\hyphenation{waar-schijn-lijk-heids-am-pli-tu-des}
\hyphenation{snel-heden}
\hyphenation{kun-nen}
\hyphenation{ver-vals-pa-den}
\hyphenation{pro-duc-tie}
\hyphenation{bij-dra-gen}
\hyphenation{in-tro-du-ceert}
\hyphenation{oor-sprong}
\hyphenation{be-schrij-ven}
\hyphenation{hoe-veel-he-den}
\hyphenation{wor-den}
\hyphenation{po-si-ties}
\hyphenation{ge-com-bi-neer-de}
\hyphenation{pro-duc-tie}
\hyphenation{pa-ra-me-ter-waar-den}
\hyphenation{am-pli-tu-de}
\hyphenation{pa-ra-me-ters}
\hyphenation{ver-schil-lende}
\hyphenation{ver-schil-len}
\hyphenation{ver-wacht}
\hyphenation{ver-wach-ting}
\hyphenation{re-so-lu-ties}
\hyphenation{re-ke-ning}
\hyphenation{ge-hou-den}
\hyphenation{ne-gen-tig-dui-zend}
\hyphenation{schat-ting}
\hyphenation{schat-ting-en}
\hyphenation{voor-spel-ling-en}
\hyphenation{Stan-daard-mo-del}
\hyphenation{mo-ge-lijk-he-den}
\hyphenation{sta-tis-tische}
\hyphenation{ver-za-me-ling}
\hyphenation{on-ze-ker-he-den}
\hyphenation{in-ter-pre-te-ren}
\hyphenation{waar-schijn-lijk}
\hyphenation{waar-schijn-lijk-heid}
\hyphenation{voor-spel-ling-en}
\hyphenation{ex-pe-ri-men-te-le}
\hyphenation{be-lang-rijke}
\hyphenation{im-puls-rich-ting-en}
...
\renewcommand{\textgg}{\unitsp≫\unitsp}
...
\newcommand{\ccd}{\text{c}\qbar[c]\text{d}}
\newcommand{\ccs}{\text{c}\qbar[c]\text{s}}
\newcommand{\btoccd}{\text{b}\to\ccd}
...
\newcommand{\fzeroalt}{\text{f}_\text{0}}
\newcommand{\ftwoalt}{\text{f}_\text{2}}
...
\newcommand{\BsbartoJpsiphi}{$\Bsbar \to \Jpsiphi$}
...
\newcommand{\BstoJpsiKS}{$\Bs\to\Jpsi\,\KS$}
...
\newcommand{\BdtoJpsirho}{$\Bd\to\Jpsi\,\rhoz$}
...
\newcommand{\BstoDspDsm}{$\Bs \to \Dsmes[+]\,\Dsmes[--]$}
...
\newcommand{\effta}{\eff(t,\Omega)}
\newcommand{\efft}{\eff_t(t)}
\newcommand{\effa}{\eff_a(\Omega)}
\newcommand{\meff}{\langle\eff\rangle}
\newcommand{\meffa}{\langle\eff_a\rangle}
\newcommand{\mefft}{\langle\eff_t\rangle}
...
\newcommand{\qtOS}{{\qt^\text{o}}}
\newcommand{\qtSS}{{\qt^\text{s}}}
...
\newcommand{\wTag}[1][]{w^{#1}}
\newcommand{\wTagBar}[1][]{{\kern 0.07em \overline{\kern -0.07em w \kern -0.01em} \kern 0.01em}^{#1}}
\newcommand{\wTagOS}{\wTag[\text{o}]}
\newcommand{\wTagSS}{\wTag[\text{s}]}
\newcommand{\wTagBarOS}{\wTagBar[\text{o}]}
\newcommand{\wTagBarSS}{\wTagBar[\text{s}]}
\newcommand{\etaTag}[1][]{\eta^{#1}}
\newcommand{\etaOS}{\etaTag[\text{o}]}
\newcommand{\etaSS}{\etaTag[\text{s}]}
\newcommandx{\pTag}[2][1=, 2=]{p_{#1}^{#2}}
\newcommandx{\pTagBar}[2][1=, 2=]{{\kern 0.07em \overline{\kern -0.07em p \kern -0.01em} \kern 0.01em}_{#1}^{#2}}
\newcommandx{\dil}[2][1=, 2=]{\mathcal{D}_{#1}^{#2}}
\newcommand{\dilOS}[1][]{\dil[#1][\text{o}]}
\newcommand{\dilSS}[1][]{\dil[#1][\text{s}]}
\newcommand{\AProd}{\nu_\text{p}}
\newcommand{\ANorm}{\nu_\text{m}}
\newcommand{\ATagEff}[1][]{\delta_{#1}}
\newcommandx{\ADilWTag}[2][1=, 2=]{\Delta_{#1}^{#2}}
\newcommand{\ADilWTagOS}[1][]{\ADilWTag[#1][\text{o}]}
\newcommand{\ADilWTagSS}[1][]{\ADilWTag[#1][\text{s}]}
