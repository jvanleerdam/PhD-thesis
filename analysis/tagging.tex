...
decay-time distributions are used to measure the a priori unknown tagging parameters. The wrong-tag probabilities for untagged decays are
given by $\wTag$\textequiv$\wTagBar$\textequiv0.5 by definition.
...
defined as $\tfrac{1}{2}(\pTag[i]+\pTagBar[i])$ and $\pTag[i]-\pTagBar[i]$, respectively. The values measured in the calibration
procedure~\cite{LHCb-ANA-2014-039} and the values from the fit are shown in Table~\ref{tab:tagCalib}. The fit values are only marginally
